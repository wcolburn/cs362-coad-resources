...
RSpec.describe Ticket, type: :model do
  ...
  it "resource_category returns all tickets that match the given resource_category id" do
    open_ticket_with_rc1 = create(:ticket, closed:false, region_id:db_region_1.id, resource_category_id:db_resource_category_1.id)
    open_ticket_with_rc2 = create(:ticket, closed:false, region_id:db_region_2.id, resource_category_id:db_resource_category_2.id)
    closed_ticket_with_rc1 = create(:ticket, closed:true, region_id:db_region_1.id, resource_category_id:db_resource_category_1.id)
    expect(Ticket.resource_category db_resource_category_1.id).to eq([open_ticket_with_rc1, closed_ticket_with_rc1])
  end
